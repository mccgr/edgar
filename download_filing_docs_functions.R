...
# Functions ----
get_file_list <- function(num_files = Inf, filing_subset = NULL) {

    # Arguments:
    # - num_files = maximum number of files to be collected and returned on list
    # - filing_subset = data frame which contains a list of file names in a column file_name.
...
download_filing_docs <- function(max_files = Inf, filing_subset = NULL) {
    ...
    new_table <- !dbExistsTable(pg, c("edgar", "filing_docs_processed"))
    dbDisconnect(pg)
    while (nrow(files <- get_file_list(num_files = max_files, filing_subset = filing_subset))>0) {
        print("Getting files...")
